...
# Constants
CHUNK_DURATION_MS = 20   # Typical Twilio chunk duration (~20ms)
SILENCE_THRESHOLD = 500  # Amplitude threshold for silence, may need tuning
SILENCE_DURATION_MS = 2000  # 2 seconds
SILENCE_CHUNKS_REQUIRED = SILENCE_DURATION_MS // CHUNK_DURATION_MS  # 100 chunks if each ~20ms
...
class MediaStream:
    def __init__(self, websocket: WebSocket, aai_client: AssemblyAIClient):
        ...
        self.received_audio = False  # Track if we have received any non-silent audio
    ...
    async def handle_media_event(self, data: dict):
        ...
        # Store the original message for playback
        self.messages.append(data)

        # If we have received audio and now have 2s of silence, stop and transcribe.
